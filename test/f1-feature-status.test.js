...
test('promoting F1 does not promote other parent feature statuses (F2, F2B, F3, F3B, F4, F4A, F5, F5A, F6, F7 remain not_implemented)', () => {
  const reqs = readRequirements();
  ...
  for (const [id, next] of guards) {
    ...
    assert.equal(
      ...
      'not_implemented',
      `${id} parent feature status must remain not_implemented — only F1 is being promoted in this change`,
    );
  }
});
